...
export class AppointmentListComponent {
  ...
  ngOnInit() { this.getAllAppointments(); this.loadRelations(); }
  ...
  loadRelations() { this.doctorService.getAllDoctors().subscribe(data => this.doctors = data); this.patientService.getAllPatients().subscribe(data => this.patients = data); }
  getDoctorName(id: number) { let d = this.doctors.find(x => x.id == id); return d ? 'Dr. ' + d.firstName + ' ' + d.lastName : id; }
  getPatientName(id: number) { let p = this.patients.find(x => x.id == id); return p ? p.firstName + ' ' + p.lastName : id; }
}
