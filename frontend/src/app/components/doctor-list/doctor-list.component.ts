...
export class DoctorListComponent {
  ...
  bookAppointment(doctorId: number, patientId: number) { this.appointmentService.bookAppointment(doctorId, patientId).subscribe(() => { this.router.navigate(['/appointments']); }); }
  deleteDoctor(id: number) { this.doctorService.deleteDoctor(id).subscribe(() => this.getAllDoctors()); }
}
