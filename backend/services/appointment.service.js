...
function getByPatient(patientId, callback) {
  db.all(
...
function bookAppointment(doctorId, patientId, callback) {
  db.get("SELECT * FROM doctors WHERE id = ?", [doctorId], (err, doctor) => {
    ...
    if (doctor.available !== 1)
      return callback(null, null, "Doctor is not available");
    db.get(
      ...
      (err2, patient) => {
        ...
        db.run(
          "INSERT INTO appointments (doctorId, patientId, appointmentDate, endDate) VALUES (?, ?, ?, ?)",
          [doctorId, patientId, appointmentDate, null],
          function (err3) {
            ...
            db.run(
              "UPDATE patients SET appointmentsCount = appointmentsCount + 1 WHERE id = ?",
              [patientId],
            );
  ...
  db.get(
    ...
    (err, appointment) => {
      ...
      if (appointment.endDate)
        return callback(null, null, "Appointment already completed");
      const endDate = today();
      db.run(
        "UPDATE appointments SET endDate = ? WHERE id = ?",
        [endDate, id],
        (err2) => {
          if (err2) return callback(err2);
          db.run("UPDATE doctors SET available = 1 WHERE id = ?", [
            appointment.doctorId,
          ]);
          db.run(
            "UPDATE patients SET appointmentsCount = CASE WHEN appointmentsCount > 0 THEN appointmentsCount - 1 ELSE 0 END WHERE id = ?",
